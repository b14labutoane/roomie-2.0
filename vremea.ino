...
const char* THINGSPEAK_API_KEY = "ZCB5RT7ANFKMJ1WU";
...
void sayHi(OLEDDisplay *display);
...
void updateData(OLEDDisplay *display) {
  ...
  currentWeatherClient.setMetric(IS_METRIX);
  currentWeatherClient.setLanguage(OPEN_WEATHER_MAP_LANGUAGE);
  currentWeatherClient.updateCurrentById(&currentWeather, OPEN_WEATHER_MAP_APP_ID, OPEN_WEATHER_MAP_LOCATION_ID);
  drawProgress(display, 50, "Updating forecasts...");
  forecastClient.setMetric(IS_METRIX);
...
void drawCurrentWeather(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x, int16_t y) {
  ...
  display->setFont(ArialMT_Plain_24);
  display->setTextAlignment(TEXT_ALIGN_LEFT);
  String temp = String(currentWeather.temp, 1) + (IS_METRIX ? "°C" : "°F");
...
void drawForecastDetails(OLEDDisplay *display, int x, int y, int dayIndex) {
  ...
  String temp = String(forecasts[dayIndex].temp, 0) + (IS_METRIX ? "°C" : "°F");
  display->setFont(ArialMT_Plain_10);
